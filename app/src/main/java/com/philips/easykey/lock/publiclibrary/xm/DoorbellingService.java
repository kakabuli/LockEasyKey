    ...
    private void sendAlarmNotification(MqttData mqttData) {
        try {
            ...
            if (mDoorbellingResult != null) {
                ...
                String title = "";
                String timestamp = "";
                if (mDoorbellingResult.getEventtype().equals(MqttConstant.VIDEO_LOCK_DOORBELLING) || mDoorbellingResult.getEventtype().equals(MqttConstant.VIDEO_LOCK_INFO_OPEN)) {
                    ...
                    if(mDoorbellingResult.getEventtype().equals(MqttConstant.VIDEO_LOCK_INFO_OPEN)){
                        title = getString(R.string.lock_state);
                        content = getString(R.string.lock_state_open);
                    }
                    if (!AppUtil.isAppOnForeground(DoorbellingService.this)) {
                        if (!content.isEmpty())
                            NotificationUtils.sendNotification(DoorbellingService.this, title, content + " " + timestamp, R.mipmap.ic_launcher, intent);
